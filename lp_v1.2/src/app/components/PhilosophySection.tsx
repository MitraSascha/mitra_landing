...
export function PhilosophySection() {
  ...
  useEffect(() => {
    ...
    cardsRef.current.forEach((card, index) => {
      ...
      // Trigger 1 – Eingang: Karte zoomt auf dich zu sobald sie im Viewport erscheint (ALLE Karten)
      gsap.fromTo(
        card,
        { scale: 1 },
        {
      ...
      // Trigger 2 – Exit: Pin + weiterzoomen + verschwinden (nur wenn nächste Karte existiert)
      ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          ...
          gsap.to(card, {
            scale: 1.14 + progress * 0.36,
            opacity: Math.max(0, 1 - progress * 2.4),
            filter: `blur(${progress > 0.3 ? (progress - 0.3) * 28 : 0}px)`,
            duration: 0.3,
            overwrite: 'auto',
          });
        },
    ...
    <section ref={sectionRef} className="relative overflow-hidden bg-gradient-to-br from-[#2c4a5f] via-[#1a3a4a] to-[#0d1f2d] py-32 md:py-40">
      ...
      {planningSteps.map((step, index) => (
        <div
          ...
          className="min-h-[60vh] md:min-h-screen flex items-center justify-center px-8 md:px-16 lg:px-24 overflow-hidden"
          style={{
