...
export function GallerySection() {
  ...
  const handleCardInteraction = (id: number) => {
    // On mobile (touch): toggle testimonial; on desktop hover handles it
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10">
        {bathrooms.map((bathroom, index) => (
          <motion.div
            ...
            onClick={() => handleCardInteraction(bathroom.id)}
          ...
          >
            <div className="relative rounded-[2.5rem] overflow-hidden shadow-2xl border border-[#2c4a5f]/10 bg-white">
              ...
              {/* Mobile tap hint - only visible when not showing testimonial */}
              <motion.div
                initial={false}
                animate={{ opacity: hoveredId === bathroom.id ? 0 : 1 }}
                transition={{ duration: 0.3 }}
                className="absolute bottom-4 right-4 md:hidden pointer-events-none"
              >
                <div
                  className="px-3 py-1.5 rounded-full font-outfit text-xs font-semibold"
                  style={{
